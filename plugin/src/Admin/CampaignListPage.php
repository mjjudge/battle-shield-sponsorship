...
class CampaignListPage {
    ...
    public function render(): void {
        ...
        echo '<div class="wrap">';
        echo '<h1>' . esc_html__( 'Campaigns', 'battle-shield-sponsorship' ) . '</h1>';
        echo '<p><a class="button button-primary" href="' . esc_url( admin_url( 'admin.php?page=bss-campaign-edit' ) ) . '">'
            . esc_html__( 'New Campaign', 'battle-shield-sponsorship' ) . '</a></p>';

        if ( isset( $_GET['activated'] ) ) {
            echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__( 'Campaign activated.', 'battle-shield-sponsorship' ) . '</p></div>';
        }
        if ( isset( $_GET['saved'] ) ) {
            echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__( 'Campaign saved.', 'battle-shield-sponsorship' ) . '</p></div>';
        }

        echo '<table class="widefat striped">';
        echo '<thead><tr>';
        echo '<th>' . esc_html__( 'Name', 'battle-shield-sponsorship' ) . '</th>';
        echo '<th>' . esc_html__( 'Event date', 'battle-shield-sponsorship' ) . '</th>';
        ...
        if ( empty( $campaigns ) ) {
            echo '<tr><td colspan="5">' . esc_html__( 'No campaigns yet.', 'battle-shield-sponsorship' ) . '</td></tr>';
        }
        ...
        foreach ( $campaigns as $campaign ) {
            ...
            echo '<td>' . esc_html( $this->fmt_date( (string) ( $campaign->event_date ?? '' ) ) ) . '</td>';
